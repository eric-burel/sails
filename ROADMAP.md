...
Feature                                          | Summary
 ...
 Keep error data on production using an argument | In order to keep the "data" argument of sails preconfigured response in production, one must change the global config "keepResponseErrors" to true. Instead, it should be passed as an attribute of the "options" argument, so that we can keep the response data on production only on selected responses.It would be particularly when we want to add custom error message designed for the end user on some response but still hide other errors in production.
